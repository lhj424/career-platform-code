...
public interface CandidateScoreService {
    ...
    /**
     * V2 新增：按岗位技能要求精确匹配（ID 匹配 + 岗位权重）
     * @param userId 学生用户ID
     * @param jobSkills 岗位技能要求列表（含skillId + 等级 + 权重）
     * @param positionTitle 岗位名称（用于测评维度映射）
     * @param filters 学校/学历/城市筛选信息
     * @return 评分结果
     */
    CandidateScore computeForJob(Long userId, List<JobSkillRequirement> jobSkills,
                                  String positionTitle, FilterInfo filters);
