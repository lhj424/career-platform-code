...
public class AdminServiceImpl implements AdminService {
    ...
    public DashboardResponse getDashboard() {
        ...
        long activeUsers = userMapper.selectCount(
                new LambdaQueryWrapper<User>().eq(User::getIsDeleted, 0).eq(User::getStatus, "ACTIVE"));
        // 已禁用用户
        long disabledUsers = totalUsers - activeUsers;
        // 各角色数量
        long studentCount = userMapper.selectCount(
                new LambdaQueryWrapper<User>().eq(User::getIsDeleted, 0).eq(User::getRole, "STUDENT"));
        long hrCount = userMapper.selectCount(
                new LambdaQueryWrapper<User>().eq(User::getIsDeleted, 0).eq(User::getRole, "HR"));
        long adminCount = userMapper.selectCount(
                new LambdaQueryWrapper<User>().eq(User::getIsDeleted, 0).eq(User::getRole, "ADMIN"));
        // 岗位总数
        long totalJobs = jobPositionMapper.selectCount(
                new LambdaQueryWrapper<JobPosition>().eq(JobPosition::getIsDeleted, 0));
