...
import com.xuelian.career.dto.request.HrInitiateRequest;
...
public class ChatController {
    ...
    /** HR 主动发起与候选人的会话 */
    @PostMapping("/conversations/hr-initiate")
    public Result<ConversationResponse> hrInitiate(@RequestBody HrInitiateRequest request,
                                                   HttpServletRequest httpRequest) {
        Long hrId = (Long) httpRequest.getAttribute("userId");
        String role = (String) httpRequest.getAttribute("role");
        if (!"HR".equals(role)) {
            return Result.forbidden("仅企业HR可以发起聊天");
        }
        // HR发起：studentId是候选人，hrId是自己
        ConversationResponse resp = chatService.createOrGetConversation(
                request.getStudentId(), hrId, request.getJobId(),
