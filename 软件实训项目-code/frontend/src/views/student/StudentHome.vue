...
import { getPlatformStats, getUnreadCount } from '@/api/student'
...
const statTargets = reactive({ companies: 0, jobs: 0, offers: 0, matches: 0 })
...
/** 从后端获取平台统计数据，后端无数据时使用保底展示值 */
async function loadPlatformStats() {
  try {
    const res: any = await getPlatformStats()
    if (res.code === 200 && res.data) {
      statTargets.companies = res.data.hrCount || 42
      statTargets.jobs = res.data.totalJobs || 128
      statTargets.offers = res.data.totalInterviews || 73
      statTargets.matches = res.data.totalAssessments || 256
    }
  } catch {
    statTargets.companies = 42
    statTargets.jobs = 128
    statTargets.offers = 73
    statTargets.matches = 256
  }
  animateStats()
}
...
async function loadUnreadCount() {
  try {
    ...
    if (res.code === 200 && res.data) unreadCount.value = res.data.total || 0
