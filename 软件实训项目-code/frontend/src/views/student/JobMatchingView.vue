...
async function handleSearch() {
  ...
  try {
    ...
    console.error('[JobMatchingView] 搜索失败:', e)
    error.value = e?.response?.data?.message || e?.message || '加载岗位失败，请稍后重试'
  } finally {
    loading.value = false
  }
}
