...
async function save() {
  loading.value = true
  try {
    // 保存前先将输入框中未提交的内容也加入标签列表
    addSkill()
    // 提交时只发送后端需要的字段，避免多余字段干扰
    const payload = {
      school: form.school,
      major: form.major,
      education: form.education,
      grade: form.grade,
      skillTags: JSON.stringify(skills.value),
      targetRoles: JSON.stringify([]), // 保留结构
      expectedCity: form.expectedCity,
      expectedSalary: form.expectedSalary,
      jobStatus: form.jobStatus,
      summary: form.summary
    }
    const res = await profileStore.saveProfile(payload)
    // 保存后更新本地 form 数据，确保与服务端一致
    if (res?.data) {
      Object.assign(form, res.data)
      skills.value = JSON.parse(res.data.skillTags || '[]')
    }
    ElMessage.success('保存成功')
  } catch (e: any) {
    const msg = e?.response?.data?.message || e?.message || '保存失败，请重试'
    ElMessage.error(msg)
    console.error('[ProfileSave] 保存失败:', e)
