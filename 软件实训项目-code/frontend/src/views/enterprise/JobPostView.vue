  ...
  <div class="job-post-page">
        ...
        >
          ...
          <div class="jc-actions">
            <el-button
              ...
              round
              plain
              @click="$router.push(`/enterprise/talent-match/${job.id}`)"
            >
              <el-icon style="margin-right:4px"><Aim /></el-icon>人才匹配
            </el-button>
            <el-button
              v-if="job.publishStatus === 1"
              type="primary"
    ...
    >
      <div class="dialog-body">
        ...
        <!-- 技能要求 -->
        <div class="dialog-section">
          <div class="dialog-section-label">技能要求</div>
          <p class="section-hint">选择该岗位需要掌握的核心技能，可设置每条技能的掌握等级和权重</p>

          <!-- 已选技能列表 -->
          <div v-if="selectedSkills.length > 0" class="skills-selected-list">
            <div
              v-for="(item, idx) in selectedSkills"
              :key="idx"
              class="skill-requirement-row"
            >
              <div class="skill-name-badge">
                <el-icon :size="14" color="#165DFF"><StarFilled /></el-icon>
              ...
              <div class="skill-level-weight">
                <el-select
                  v-model="item.requiredLevel"
                  placeholder="等级"
                  size="small"
                  style="width:90px"
                >
                ...
                <el-select
                  ...
                  style="width:90px"
                >
                  <el-option label="⭐ 重要" :value="1.0" />
                  <el-option label="⭐⭐ 核心" :value="2.0" />
                  <el-option label="⭐⭐⭐ 必须" :value="3.0" />
                </el-select>
              </div>
            </div>
          </div>
          <div v-else class="skills-empty-hint">
            暂未添加技能要求，请从下方技能库中选择
          </div>
          ...
          <div class="skill-add-row">
            <el-select
              ...
              filterable
              clearable
              placeholder="搜索并添加技能（如：Java、Python、Spring Boot…）"
              size="default"
              style="width:100%"
              @change="onSkillSelect"
              :filter-method="(val: string) => skillSearchKeyword = val"
            >
              <el-option
                v-for="sk in availableSkills"
                :key="sk.id"
                :label="`${sk.name} (${sk.category || '其他'})`"
                :value="sk.id"
              />
            </el-select>
          </div>
...
import {
  ...
  Money, Clock, Edit, ArrowDown, FolderDelete, StarFilled, Close, Aim
} from '@element-plus/icons-vue'
import { listMyJobs, publishJob, updateJob, offlineJob, republishJob, deleteJob, uploadJobImage, getAllSkills, getJobSkills } from '@/api/enterprise'
...
// 技能相关
const skillLevels = ['了解', '掌握', '熟练', '精通']
const skillLibrary = ref<any[]>([])            // 技能库
const selectedSkills = ref<any[]>([])           // 当前对话框选中的技能 {skillId, requiredLevel, weight}
const skillSearchKeyword = ref('')              // 技能搜索关键词
...
async function loadSkillLibrary() {
  if (skillLibrary.value.length > 0) return
  try {
    const res: any = await getAllSkills()
...
const availableSkills = computed(() => {
  const selectedIds = selectedSkills.value.map(s => s.skillId)
  return skillLibrary.value.filter((sk: any) => !selectedIds.includes(sk.id))
})

// 获取技能名称
function getSkillName(skillId: number): string {
  const sk = skillLibrary.value.find((s: any) => s.id === skillId)
  return sk ? sk.name : '未知技能'
}

// 添加技能
function onSkillSelect(skillId: number) {
  if (!skillId) return
  if (selectedSkills.value.some(s => s.skillId === skillId)) {
...
function buildSkillRequirements() {
  return selectedSkills.value.map(s => ({
    skillId: s.skillId,
...
function openDialog(job: any | null) {
  ...
  if (job) {
    ...
    // 加载该岗位已有技能要求
    getJobSkills(job.id).then((res: any) => {
      const skills = res.data || []
      selectedSkills.value = skills.map((s: any) => ({
        skillId: s.skillId,
        requiredLevel: s.requiredLevel || '了解',
        weight: s.weight || 1.0
      }))
    }).catch(() => {
      selectedSkills.value = []
...
async function submitForm() {
  ...
  try {
    const submitData = {
      ...form,
      skillRequirements: buildSkillRequirements()
    }
    if (editingJob.value) {
      await updateJob(editingJob.value.id, submitData)
...
/* 技能要求样式 */
.section-hint {
  font-size: 12px; color: #86909c; margin: -8px 0 12px; padding-left: 0;
}
...
.skill-requirement-row {
  ...
  border: 1px solid #f0f0f0; gap: 8px;
}
.skill-name-badge {
...
.skill-remove-btn {
  width: 20px; height: 20px; padding: 0; min-width: 20px; font-size: 10px;
}
.skill-level-weight {
  display: flex; gap: 8px; flex-shrink: 0;
}
.skills-empty-hint {
  padding: 16px; text-align: center; color: #c0c4cc;
  font-size: 13px; border: 1px dashed #e5e6eb; border-radius: 8px;
  margin-bottom: 14px;
}
.skill-add-row {
  margin-top: 0;
