          ...
          <div v-for="cand in result.candidates" :key="cand.userId || cand.username" class="rm-cand-card">
            <div class="rmcc-header">
              ...
              <div class="rmcc-right">
                ...
                <el-button type="primary" size="small" round @click="handleChat(cand)">
            ...
            <div class="rmcc-scores">
              ...
              <div class="rmcc-score-row">
                <span class="rmcc-score-label">学习掌握</span>
                <div class="rmcc-score-track">
...
import { hrInitiateConversation } from '@/api/chat'
...
/** HR点击沟通：创建会话后跳转到聊天页 */
async function handleChat(cand: any) {
  try {
    const res: any = await hrInitiateConversation({
      studentId: cand.userId,
      jobId: cand.matchJobId,
      jobTitle: cand.matchJobTitle || ''
    })
    router.push({ path: '/chat', query: { conversationId: res.data.id } })
  } catch (e: any) {
