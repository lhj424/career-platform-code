          ...
          <div class="cc-main">
            ...
            <div class="cc-info">
              <div class="cc-name-row">
                ...
                <el-select
                  :model-value="cand.status"
                  size="small"
                  effect="light"
                  class="cc-status-tag"
                  @change="(val: string) => updateStatus(cand, val)"
                >
                  <el-option
                    ...
                    :value="opt.value"
                  >
                    <span class="cc-status-dot" :class="opt.value"></span>
                    {{ opt.label }}
                  </el-option>
                </el-select>
...
async function updateStatus(row: any, newStatus: string) {
  ...
  try {
    await ElMessageBox.confirm(
      `确定将 <strong>${row.realName || '该候选人'}</strong> 状态改为「${actionText}」吗？`,
      '确认操作',
      { type: 'warning', dangerouslyUseHTMLString: true }
    )
    const res: any = await updateApplicationStatus(row.id, newStatus)
    if (res.code === 200) {
      ElMessage.success(`已更新为${actionText}`)
...
.cc-status-tag { font-size: 11px; width: 88px; }
.cc-status-tag :deep(.el-input__wrapper) { padding: 0 4px; }
.cc-status-tag :deep(.el-select__selected-item) { font-size: 11px; display: flex; align-items: center; }
.cc-status-dot {
