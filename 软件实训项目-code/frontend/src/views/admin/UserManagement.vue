...
import { getUsers, updateUserStatus, resetUserPassword, getDashboard } from '@/api/admin'
...
/** 从后端 /api/admin/dashboard 获取全库用户统计数据 */
async function fetchStats() {
  try {
    ...
    disabledUsers.value = d.disabledUsers ?? 0
    studentCount.value = d.studentCount ?? 0
    hrCount.value = d.hrCount ?? 0
    adminCount.value = d.adminCount ?? 0
  } catch { /* 静默失败 */ }
}
