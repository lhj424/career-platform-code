...
async function handleLogin() {
  ...
  try {
    ...
    // setUserInfo 已同步写入 localStorage 和 Pinia，直接跳转即可
    // shouldSuppress401() 会处理任何并发的 401 时序问题
...
function redirectByRole(role: string) {
  // 使用 router.push 替代 window.location.hash，避免页面重载导致 token 时序问题
  if (role === 'STUDENT') router.push('/student/home')
  else if (role === 'HR') router.push('/enterprise/home')
